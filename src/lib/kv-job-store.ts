 ...
 */
export const clearOldJobs = async (maxAgeMinutes: number = 60): Promise<number> => {
  try {
    ...
    for (const job of jobs) {
      ...
      if (jobAge > maxAgeMs) {
        try {
          ...
          await client.sRem(JOBS_KEY, job.id);
          
          // Remove job data
          await client.del(`${JOB_PREFIX}${job.id}`);
          ...
          console.warn(`⚠️ Failed to delete old job ${job.id}:`, error);
        }
      }
    }
    
    console.log(`🗑️ Cleaned up ${deletedCount} old jobs from Redis`);
    return deletedCount;
  } catch (error) {
    console.error('❌ Error cleaning up old jobs from Redis:', error);
    return 0;
  }
};
 ...
 * Get jobs statistics
 */
export const getJobsStats = async (): Promise<{
  total: number;
  ...
  try {
    ...
    console.error('❌ Error getting jobs stats from Redis:', error);
...
export const healthCheck = async (): Promise<boolean> => {
  try {
    const client = await getRedisClient();
    const testKey = 'smartbdx:health:test';
